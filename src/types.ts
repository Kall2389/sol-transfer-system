...
export interface PhantomProvider {
  ...
  signMessage: (message: Uint8Array | string, display?: DisplayEncoding) => Promise<Uint8Array | string>;
  connect: (opts?: Partial<ConnectOpts>) => Promise<{ publicKey: PublicKey }>;
  disconnect: () => Promise<void>;
  on: (event: PhantomEvent, handler: (args: unknown) => void) => void;
  request: (method: PhantomRequestMethod, params: Record<string, unknown>) => Promise<unknown>;
}
